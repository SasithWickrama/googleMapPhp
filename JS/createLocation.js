	 ...
	 $(".frmbody").html('<![CDATA[<!DOCTYPE html>'+
 	 ...
 	 $(".frmbody").html('<![CDATA[<!DOCTYPE html>'+                     
         ...
         $(".frmbody").html('<![CDATA[<!DOCTYPE html>'+
                            ...
                            '<body>'+
                            '<table class="table" style="width:100%">'+
                            '<tr><td>MSAN Number:</td><td><input type="text" id="odfNo"/></td></tr>'+
                            '<tr><td rowspan=2>GPS location:</td><td>Longitute: '+Longitute+'</td></tr>'+
                            '<tr><td>latitude: '+Latitude+'</td></tr>'+
							'<tr style="display:none"><td><input type="text" id="msanlng" value="'+Longitute+'"/></td><td><input type="text" id="msanlat" value="'+Latitude+'"/></td></tr>'+
                            '<tr><td></td><td><button type="button" id="saveMsanData" class="btn btn-primary btn-sm" style="float:right"><i class="glyphicon glyphicon-folder-close"></i>&nbsp;&nbsp;Save</button></td></tr>'+  
                            '</table>'+
                            '</body>'+
                            '</html>');
...
$(document).on('click','#saveductData',function(){
		...
		if(cmbNumwaysval == 'etc'){
			...
			var ductways = document.getElementById('ductwayscmb').options[document.getElementById('ductwayscmb').selectedIndex].value;
...
function updateductpldata(){  
    ...
    var id = event.target.id.replace("save", "");
	var mhend1 = document.getElementById(id+'mhend1').value;
	var mhend2 = document.getElementById(id+'mhend2').value;
	
	if(mhend1 ==''){
	
		swal("", "Manhole END-1 required!", "warning");

		return;

	}else if(mhend2==''){
	
		swal("", "Manhole END-2 required!", "warning");
...
function deleteductpldata(){  
  ...
  swal({
    title: "Are you sure?",
    text: "Once deleted,DUCT informations will not be able to recover!",
    icon: "warning",
    buttons: true,
    dangerMode: true,
  })
  .then((willDelete) => {
    if (willDelete) {
      swal("DUCT informations have been deleted", {
        icon: "success",
      });
...
function updateFDPpldata(){  
    ...
    var info =[];
    info[0] =   id;
    info[1] =   document.getElementById(id+'leaname').value;
    info[2] =   document.getElementById(id+'rdname').value;
    info[3] =   document.getElementById(id+'splitter').value;
    info[4] =   document.getElementById(id+'stype').value;
...
function updateFTCpldata(){  
	...
	info[1] =   document.getElementById(id+'leaname').value;
	info[2] =   document.getElementById(id+'rdname').value;
	info[3] =   document.getElementById(id+'side').value;
	info[4] =   document.getElementById(id+'type').value;
	info[5] =   document.getElementById(id+'capacity').value;
	info[6] =   document.getElementById(id+'feature').value;

	$.ajax({
	   type: "POST",
	   data: {info:info},
	   url: "./function.php?q=83",
	   success: function(msg){
...
function updateFJpldata(){  

	var id = event.target.id.replace("save", "");
    
    var info =[];
    info[0] =   id;
	info[1] =   document.getElementById(id+'connumber').value;
	info[2] =   document.getElementById(id+'type').value;
	info[3] =   document.getElementById(id+'snumber').value;

	$.ajax({
	   type: "POST",
	   data: {info:info},
	   url: "./function.php?q=84",
	   success: function(msg){
...
function updateODFpldata(){  
    ...
    info[0] =   id;
	info[1] =   document.getElementById(id+'leaname').value;
	info[2] =   document.getElementById(id+'lblocation').value;
	info[3] =   document.getElementById(id+'lac').value;
	info[4] =   document.getElementById(id+'rnumber').value;
	info[5] =   document.getElementById(id+'shname').value;
	info[6] =   document.getElementById(id+'numberofct').value;

	$.ajax({
	   type: "POST",
	   data: {info:info},
	   url: "./function.php?q=85",
	   success: function(msg){
...
function TransCbldata(){	
	...
	$.ajax({
       type: "POST",
       data: {info:info},
       url: "./function.php?q=75",
       success: function(msg){
       swal("Process Success");
       //cbl.hideDocument();
       getdatacable();
       }
    });
  ...
  .then((willDelete) => {
    if (willDelete) {
      swal("All cable informations have been deleted", {
       ...
       type: "POST",
       data: {cblNo:cblNo},
       url: "./function.php?q=76",
       success: function(msg){
       //allNetwork.hideDocument();
       getallnetdata();
       }
    });
...
$(document).on('click','#saveCableData',function(){
		
		var rtom = document.getElementById("rtom").value;
		
		if(rtom == ''){

			swal("", "Plese select a RTOM to continue!", "warning");
			

			return;
		}
	
		if($('#createcableName').val() == ''){

			swal("", "Cable name required!", "warning");
		...
		var info =[];
		info[0] = 	document.getElementById('createcableName').value;
		info[1] = 	document.getElementById('createcableType').value;
		info[2] = 	document.getElementById('createcable_mhend1').value;
		info[3] = 	document.getElementById('createcable_mhend2').value;
		info[4] = 	document.getElementById('length').value;
		info[5] = 	document.getElementById('cablecores').value;
		info[6] = 	document.getElementById('dname').value;
		info[7] = 	document.getElementById('rtom').value;
		info[8] = 	document.getElementById('c_cordinates').value;


		$.ajax({
		   type: "POST",
		   data: {info:info},
		   url: "./function.php?q=77",
		   success: function(msg){

		   	 $("#myModal").modal('hide');
		   	 swal("Successful", "You created new Cable!", "success");
